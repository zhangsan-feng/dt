...
async def gen_ms_token(user_agent=None):
    request_headers = {
        ...
        'user-agent': user_agent or "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 Edg/126.0.0.0",
    }

    request_url = "https://mssdk.bytedance.com/web/report"
    ...
    try:
        ...
        logger.warning("Douyin msToken request failed error=%s", type(exc).__name__)
        return ""
    ...
    if not token:
        ...
        token = cookie.get("msToken").value if cookie.get("msToken") else ""
    logger.info(
        "Douyin msToken response status=%s content_type=%s token_present=%s",
